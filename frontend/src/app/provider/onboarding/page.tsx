...
export default function ProviderOnboardingPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.log('[Onboarding] Creating provider profile...');

            // Crear el perfil de proveedor (el usuario ya debería estar sincronizado por useClerkSync)
            const providerResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/providers`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    latitude: -33.4489, // Default Santiago coordinates
                    longitude: -70.6693,
                    ...
                    minAdvanceBooking: 60, // 1 hora de anticipación mínima
                    maxAdvanceBooking: 10080, // 7 días de anticipación máxima
                }),
            });

            if (!providerResponse.ok) {
                const contentType = providerResponse.headers.get('content-type');
                let errorMessage = 'Error al crear perfil de proveedor';

                if (contentType && contentType.includes('application/json')) {
                    const errorData = await providerResponse.json();
                    errorMessage = errorData.message || errorMessage;

                    // Si el usuario ya tiene un proveedor registrado, redirigir al dashboard
                    if (errorMessage.includes('ya tiene un proveedor')) {
                        console.log('[Onboarding] User already has a provider, redirecting...');
                        router.push('/dashboard?provider_request=pending');
                        return;
                    }
                } else {
            ...
            console.log('[Onboarding] Provider profile created:', providerData);

            // Actualizar el rol del usuario a PROVIDER directamente con /users/me
            console.log('[Onboarding] Updating user role to PROVIDER...');
            const roleResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/users/me`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
            ...
            if (!roleResponse.ok) {
                console.error('[Onboarding] Failed to update role:', await roleResponse.text());
            } else {
                console.log('[Onboarding] Role updated to PROVIDER');
            }

            // Actualizar el metadata en Clerk para que el rol se refleje inmediatamente
            await user.update({
                unsafeMetadata: {
            ...
            // Recargar el usuario para obtener el nuevo metadata
            await user.reload();

            // Redirigir al dashboard del cliente con mensaje de solicitud enviada
            // El usuario seguirá siendo CUSTOMER hasta que el admin apruebe la solicitud
            router.push('/dashboard?provider_request=pending');
        } catch (error) {
            console.error('[Onboarding] Error:', error);
            setErrors({ submit: error instanceof Error ? error.message : 'Error al registrar. Intenta de nuevo.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <>
            <ModernNavbar />
            <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-12 px-4 lg:pl-72">
